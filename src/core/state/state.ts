...
export class State {
  ...
  getMatchingState(id: string): IState {
    ...
    if (id) {
      state = this.sessions.find(
        (session: SessionState) => session.identifier === id
      );
      if (state === undefined) {
    ...
    return Object.keys(bodyMatcher).every((key) => {
      if (typeof bodyMatcher[key] === "object") {
        return body[key] !== undefined
          ? this.matchesBody(bodyMatcher[key], body[key])
          : false;
      }
  ...
  setToDefaults(id: string): void {
    ...
    Object.keys(state.mocks).forEach((mockName) => {
      state.mocks[mockName] = JSON.parse(
        JSON.stringify(this.defaults[mockName])
      );
    });
  }
